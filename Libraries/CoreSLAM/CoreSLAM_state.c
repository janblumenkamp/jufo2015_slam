    ...
    for (i = 0; i < state->laser_params.scan_size; i++) {
        for (j = 0; j != span; j++) {
            ...
            if (i > state->laser_params.detection_margin && i < state->laser_params.scan_size - state->laser_params.detection_margin) {
                if (sd->d[i] == 0) {
					scan->x[scan->nb_points] = state->laser_params.distance_no_detection * cosf(angle_rad);
					...
					scan->y[scan->nb_points] = state->laser_params.distance_no_detection * sinf(angle_rad);
                    scan->value[scan->nb_points] = TS_NO_OBSTACLE;
                    scan->nb_points++;
                }
                if (sd->d[i] > state->hole_width / 2) {
					scan->x[scan->nb_points] = sd->d[i] * cosf(angle_rad);
					scan->x[scan->nb_points] -= sd->v * 1000 * ((float)(i * span + j)) * (state->laser_params.angle_max - state->laser_params.angle_min) / (state->laser_params.scan_size * span - 1) / 3600.0;
					scan->y[scan->nb_points] = sd->d[i] * sinf(angle_rad);
...
void ts_iterative_map_building(ts_sensor_data_t *sd, ts_state_t *state)
{
    ...
    if (state->timestamp != 0) {
		...
		position.x += v * 1000 * cosf(thetarad);
		position.y += v * 1000 * sinf(thetarad);
    ...
    // Monte Carlo search
	position.x += state->laser_params.offset * cosf(thetarad);
	position.y += state->laser_params.offset * sinf(thetarad);
	...
	sd->position[state->direction].x -= state->laser_params.offset * cosf(position.theta * M_PI / 180);
	sd->position[state->direction].y -= state->laser_params.offset * sinf(position.theta * M_PI / 180);
