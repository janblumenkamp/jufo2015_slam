...
void gui_el_page_map_putInvisible(void)
{

}

void gui_el_page_view_putInvisible(void)
{

}

void gui_el_page_settings_putInvisible(void)
{
...
void gui_el_event_area_statusbar(ELEMENT_EVENT *event)
{
	if(event->released)
	{
		if(gui_element[GUI_EL_AREA_STATUSBAR_TOP].state == STAT_NOT_DROPPED)
		{
			...
			gui_el_page_map_putInvisible();
			gui_el_page_view_putInvisible();
			gui_el_page_settings_putInvisible();
...
void gui_init(void)
{
	...
	gui_element[GUI_EL_MBTN_MAP].state = MBTN_ACTIVE;
		...
		gui_element[GUI_EL_AREA_MAP].length = 300;
		gui_element[GUI_EL_AREA_MAP].y = STATUSBAR_HEIGHT + gui_element[GUI_EL_MBTN_MAP].heigth + 1;
		gui_element[GUI_EL_AREA_MAP].heigth = GetMaxY() - gui_element[GUI_EL_AREA_CONTENT].y;
		gui_element[GUI_EL_AREA_MAP].state = GUI_EL_INTOUCHABLE;

		gui_element[GUI_EL_AREA_SCAN].x = 301;
		gui_element[GUI_EL_AREA_SCAN].length = GetMaxX() - gui_element[GUI_EL_AREA_SCAN].x;
		gui_element[GUI_EL_AREA_SCAN].y = STATUSBAR_HEIGHT + gui_element[GUI_EL_MBTN_MAP].heigth + 1;
		gui_element[GUI_EL_AREA_SCAN].heigth = GetMaxY() - gui_element[GUI_EL_AREA_CONTENT].y;
		gui_element[GUI_EL_AREA_SCAN].state = GUI_EL_INTOUCHABLE;
...
portTASK_FUNCTION( vGUITask, pvParameters )
{
	...
	for(;;)
	{
		...
		case MENU_MAP_IDLE:
			...
			gui_drawAREAscan(&gui_element[GUI_EL_AREA_SCAN]);
